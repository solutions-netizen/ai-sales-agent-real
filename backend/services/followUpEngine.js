...
// Touch schedule: [touchNumber, delayMs, channel, templateFn]
function buildSequence(lead) {
  ...
  return [
    ...
    },
    {
      ...
      channel: "sms",
      message: `Last check-in, ${name}! ${AGENT_NAME} here. If now isn't the right time, no worries — just reply REMOVE to opt out, or CALL ME when you're ready. Wishing you well! 😊`,
    },
  ];
}
...
// Schedule all follow-up touches for a newly created lead.
// Uses setTimeout — works in a single-process Node server.
// For production, swap with a proper job queue (Bull, pg-boss, etc.)
export async function scheduleFollowUpSequence(leadId) {
  const lead = db.prepare("SELECT * FROM leads WHERE id = ?").get(leadId);
  if (!lead || !lead.phone) return; // No phone = no SMS/call sequence
  ...
  for (const touch of sequence) {
    setTimeout(async () => {
      // Re-fetch lead — if stage changed to placed/lost, stop sequence
      const current = db.prepare("SELECT * FROM leads WHERE id = ?").get(leadId);
      if (!current || ["placed", "lost"].includes(current.stage)) return;

      // Check if we already sent this touch
      ...
      try {
        if (touch.channel === "sms" && touch.message) {
          await sendSMS(lead.phone, touch.message);
          db.prepare(`
        ...
        // Update next_followup timestamp
    ...
    }, Math.min(touch.delayMs, MAX_DELAY));
  }
